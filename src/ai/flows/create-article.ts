...
const CreateArticleInputSchema = z.object({
  ...
  generateMainImage: z.boolean().optional().default(false).describe('Whether to generate a main image for the article.'),
});
...
const CreateArticleOutputSchema = z.object({
  article: z.string().describe('The generated article in Markdown format (may include a prepended image).'),
  mainImageUrl: z.string().optional().describe('Data URI of the generated main image, if requested.'),
  mainImagePrompt: z.string().optional().describe('The prompt used for generating the main image, if requested.'),
  tokenUsage: z.object({
    totalTokens: z.number().describe('Total tokens used for this operation (text + image).'),
    textGenerationTokens: z.number().optional().describe('Tokens used specifically for text generation.'),
    imageGenerationTokens: z.number().optional().describe('Tokens used specifically for image generation.'),
...
const articlePrompt = ai.definePrompt({
  ...
  input: {schema: z.object({prompt: CreateArticleInputSchema.shape.prompt })}, // Only prompt for text generation
...
const createArticleFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    const articleResult = await articlePrompt({ prompt: input.prompt });
    
    if (!articleResult.output) {
      throw new Error('Article generation failed: No output from LLM for text content.');
    }
    ...
    if (input.generateMainImage) {
      mainImageAIGenerationPrompt = `Generate a high-quality, visually appealing, and relevant main image for an article with the following topic: "${input.prompt.substring(0, 250)}${input.prompt.length > 250 ? '...' : ''}". The image should be suitable as a header or cover image for a blog post or online article.`;
      ...
      try {
        const imageGenerationResult = await ai.generate({
          ...
          config: {
            ...
            safetySettings: [ // Added safety settings to be less restrictive for DANGEROUS_CONTENT
        ...
        if (imageGenerationResult.media && imageGenerationResult.media.url) {
          ...
          console.warn('Image generation was requested but did not return a media URL. Proceeding without image.');
    ...
    return {
      ...
      tokenUsage: {
        ...
        imageGenerationTokens: imageGenerationUsage > 0 ? imageGenerationUsage : undefined,
      },
    };
  }
);
